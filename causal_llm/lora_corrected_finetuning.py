...
class CustomTrainer(Trainer):
    ...
    def compute_loss(
        ...
        **kwargs  # Accept additional keyword arguments
        ...
        loss, probabilities = custom_loss_function(logits, labels, self.processing_class)
...
def enable_grad_for_special_tokens(model, tokenizer, special_tokens):
    embedding_layer = model.get_input_embeddings()
    
    # Ensure the whole embedding layer is initially frozen
    embedding_layer.weight.requires_grad = False

    # Create a mask for trainable embeddings
    mask = torch.zeros_like(embedding_layer.weight, dtype=torch.bool)

    for token in special_tokens:
        token_id = tokenizer.convert_tokens_to_ids(token)
        mask[token_id] = True  # Set mask for the token to trainable

    # Temporarily enable requires_grad to register the hook
    embedding_layer.weight.requires_grad = True

    # Register a hook to enable gradient updates only for the masked embeddings
    def mask_gradients(grad):
        return grad * mask.float()
    
    embedding_layer.weight.register_hook(mask_gradients)

    # Re-disable requires_grad for the embedding layer
    embedding_layer.weight.requires_grad = False

    print("Enabled gradient updates for special tokens:", special_tokens)


# def enable_grad_for_special_tokens(model, tokenizer, special_tokens):
#     embedding_layer = model.get_input_embeddings()
#     embedding_layer.weight.requires_grad = False  # Freeze entire embedding layer

#     for token in special_tokens:
#         token_id = tokenizer.convert_tokens_to_ids(token)
#         # Set specific embeddings as trainable parameters
#         embedding_layer.weight.data[token_id] = torch.nn.Parameter(
#             embedding_layer.weight[token_id].clone()
#         )
